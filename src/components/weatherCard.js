  ...
  modifyData(data) {
      ...
      const currTime = new Date();
        ...
        _.forEach(result, function(value) {
            var flag = false;
            _.forEach(value.time, function(value1) {
            
                if(currTime.setDate(value1.maindate.getDate()) < value1.maindate && flag != true) {
                    flag = true;
                    value['currentTemp'] = value1.temp;
                }
            });
        });
      
        _.remove(result, function(n) {
            return !(_.has(n, 'currentTemp'));
        });
